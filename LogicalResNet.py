...
class LogicalResNet(ResNet):
    ...
    def __init__(self, block=BasicBlock, layers=[2, 2, 2, 2], tree_depth=10, num_features=10000, num_classes=1000, zero_init_residual=False,
        ...
        self.device = device
        logical_body = [nn.Linear(num_features, num_features, bias=False) for depth in range(tree_depth - 1)]
        logical_head = [nn.Linear(num_features, num_classes, bias=False)]
